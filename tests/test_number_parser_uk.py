...
class TestUkrainianPronounce(unittest.TestCase):
    ...
    def test_pronounce_negative_and_decimal(self):
        expected = {-7: 'мінус сім', -42: 'мінус сорок два', 2.5: "два крапка п'ять", 3.14: 'три крапка один чотири', -3.5: "мінус три крапка п'ять", 0.5: "нуль крапка п'ять"}
        for number, spoken in expected.items():
            with self.subTest(number=number):
                self.assertEqual(pronounce_number(number, lang="uk"), spoken)
...
class TestUkrainianExtract(unittest.TestCase):
    ...
    def test_extract_from_sentence(self):
        self.assertEqual(extract_number('у мене двадцять один котів', lang="uk"),
                         21)
